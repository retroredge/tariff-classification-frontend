...
class ReferCaseController @Inject()(actions: RequestActions,
  ...
  def referCase(reference: String): Action[AnyContent] = actions.authorisedWithWriteAccess.async { implicit request =>
  ...
  def confirmReferCase(reference: String): Action[AnyContent] = actions.authorisedWithWriteAccess.async { implicit request =>
