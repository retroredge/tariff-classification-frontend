...
class SuspendCaseController @Inject()(actions: RequestActions,
  ...
  def suspendCase(reference: String): Action[AnyContent] = actions.authorisedWithWriteAccess.async { implicit request =>
    getCaseAndRenderView(reference, c => successful(views.html.suspend_case(c, form)))
  }

  def confirmSuspendCase(reference: String): Action[AnyContent] = actions.authorisedWithWriteAccess.async { implicit request =>
